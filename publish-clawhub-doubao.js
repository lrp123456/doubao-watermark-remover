...
/**
 * ClawHub 发布脚本 v3.1.0
 * 使用 doubao-watermark-remover 名称
 */
...
try {
  ...
  -F "changelog=${encodeURIComponent(config.changelog)}" \\
  -k -s`;
  ...
  try {
    ...
    if (result.includes('success') || result.includes('OK') || result.includes('published')) {
      console.log('\n✅ 发布成功！');
      console.log(`🌐 查看：https://clawhub.ai/${config.slug}`);
      try { fs.unlinkSync(tarball); } catch(e) {}
      process.exit(0);
